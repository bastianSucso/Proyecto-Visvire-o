...
@Component({
  ...
  template: `
    <div class="space-y-6">
      <div>
        <h1 class="text-2xl font-semibold text-slate-900">Finanzas · Egresos manuales</h1>
        <p class="text-slate-500 mt-1">Registro y control de egresos administrativos.</p>
      </div>
      ...
      <div class="flex flex-wrap gap-2">
        ...
        <a class="rounded-lg border border-blue-600 bg-blue-600 px-3 py-2 text-sm text-white" routerLink="/admin/finanzas/egresos">Egresos</a>
      </div>

      <div class="rounded-xl border border-slate-200 bg-white p-4">
        <h2 class="text-lg font-semibold text-slate-900">Registrar egreso manual</h2>
        ...
        <form class="mt-4 grid grid-cols-1 md:grid-cols-2 gap-3" (ngSubmit)="crearEgreso()">
          <label class="block">
          ...
          <label class="block">
            <span class="text-sm text-slate-700">Aplica credito fiscal</span>
      ...
      <div class="rounded-xl border border-slate-200 bg-white p-4">
        ...
        <div *ngIf="loading" class="mt-4 text-sm text-slate-500">Cargando egresos...</div>
        <div *ngIf="!loading && egresos.length === 0" class="mt-4 text-sm text-slate-500">Sin registros.</div>

        <div *ngIf="!loading && egresos.length > 0" class="mt-4 overflow-x-auto">
          <table class="min-w-full text-sm">
            <thead>
              <tr class="border-b border-slate-200 text-left text-slate-500">
                ...
                <th class="py-2 pr-4 font-medium">Credito fiscal</th>
                <th class="py-2 pr-4 font-medium">Descripcion</th>
                <th class="py-2 pr-4 font-medium">Acciones</th>
              </tr>
            </thead>
            <tbody>
              <tr *ngFor="let row of egresos" class="border-b border-slate-100">
...
export class FinanzasEgresosPage implements OnInit {
  ...
  editarEgreso(row: MovimientoFinanciero) {
  ...
  eliminarEgreso(row: MovimientoFinanciero) {
    const confirmar = window.confirm('Se anulara este egreso manual. Esta accion no elimina fisicamente el registro.');
    if (!confirmar) return;
